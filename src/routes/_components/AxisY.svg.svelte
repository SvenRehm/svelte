...
<script>
	...
	const { height, xRange, yScale, width } = getContext('LayerCake');
...
<g class="axis y-axis">
	{#each tickVals as tick (tick)}
		...
		{#if baseline === true}
			<line stroke="#aaa" stroke-width="1" y1="0" y2={$height} x1="0" x2="0" />
		{/if}

		<g class="tick tick-{tick}" transform="translate({$xRange[0]}, {tickValPx})">
			{#if gridlines === true}
				<line class="gridline" {x1} x2={$width} y1={y} y2={y} />
			{/if}
			{#if tickMarks === true}
				<line class="tick-mark" stroke="#aaa" {x1} x2={x1 + tickLen} y1={y} y2={y}></line>
			{/if}
			<text
				x={x1}
				{y}
